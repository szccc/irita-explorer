<template>
  <div :style="{ opacity: show ? 1 : 0 }" ref="tableContainer">
    <div class="m-table-header">
      <table :class="['m_table', data.length > 0 ? 'm-table-header-table-fixed' : '']" cellspacing="0" cellpadding="0" border="0" ref="tableHeader">
        <colgroup>
          <col v-for="(v, i) in colWidth" :key="i" :style="{ width: columns[i] && columns[i].width ? columns[i].width + 'px' : v + 'px', minWidth: columns[i] && columns[i].minWidth + 'px', maxWidth: columns[i] && columns[i].maxWidth + 'px' }" />
        </colgroup>
        <thead>
          <tr>
            <th :class="[v.className, v.sortable ? 'sorting' : '']" @click="sortDataByClick(v)" v-for="(v, i) in columns" :key="i">
              {{ v.title }}
              <i class="sort" v-if="v.sortable" :class="{ desc: (v.key === sortAsBy || v.slot === sortAsBy) && sortAsDesc, asc: (v.key === sortAsBy || v.slot === sortAsBy) && !sortAsDesc }"></i>
            </th>
          </tr>
        </thead>
      </table>
    ...
    <div class="m-table-body">
      <table class="m_table" cellspacing="0" cellpadding="0" border="0">
        ...
        <tbody ref="table_body">
          <tr v-for="(v, i) in data" :key="i">
            <td v-for="(it, j) in columns" :width="it.width" :key="j" :class="it.className">
              <template v-if="it.key">
                <div :class="{ tooltip_span_container: it.tooltip || v[`${it.key || it.slot}_tooltip`] }" v-table-tooltip="{ show: it.tooltip || v[`${it.key || it.slot}_tooltip`], container: $refs.tableContainer }">
                  {{ v[it.key] }}
                  <div class="tooltip_span" :class="it.tooltipClassName" v-if="it.tooltip || v[`${it.key || it.slot}_tooltip`]">
                    <div>
